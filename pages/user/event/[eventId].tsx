...
function UserScoreInput({
  teamMember,
  formState,
}: {
  teamMember: TeamMember
  formState: [AddScoreForm, Dispatch<SetStateAction<AddScoreForm>>]
}) {
  ...
  return (
    <>
      <div className="w-full lg:w-6/12 px-4">
        <div className="relative mt-6 w-full mb-3">{user.name}</div>
      </div>
      <div className="w-full lg:w-6/12 px-4">
        <div className="relative w-full mb-3">
          <label className="block uppercase text-blueGray-600 text-xs font-bold mb-2">Kills</label>
          <input
            id={teamMember.user_id + '-kills'}
            type="number"
            min={0}
            max={55}
            defaultValue={0}
...
function Page({ eventId }: { eventId: string }) {
  ...
  const addScore = async () => {
    addToast('Adding score...', { appearance: 'info', autoDismiss: true }, async (toastId) => {
      ...
      Array.from(document.querySelectorAll('input')).forEach((input) => (input.value = '0'))
    })
  }
      ...
      <div className="relative max-w-4xl mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
        <div className="rounded-t bg-white mb-0 px-6 py-6">
          <div className="text-center flex justify-between">
            <h6 className="text-blueGray-700 text-xl font-bold">Event starts 17 July, 2021 at Noon EST</h6>
          </div>
        </div>
          ...
          <React.Suspense fallback={<Spinner light={false} />}>
            <form>
              ...
              <div className="flex flex-wrap">
                <div className="w-full lg:w-6/12 px-4">
                  <div className="relative mt-6 w-full mb-3">Team</div>
                </div>
                <div className="w-full lg:w-6/12 px-4">
                  <div className="relative w-full mb-3">
                    <label className="block uppercase text-blueGray-600 text-xs font-bold mb-2">Bounties</label>
                    <select
                      name="bounties"
                      id="bounties"
                      className="w-full"
                ...
                {team?.team_members
                  .filter((member) => member.member_type !== TeamMemberType.SCOREKEEPER)
                  .sort(firstBy('user_id'))
                  .map((member) => (
