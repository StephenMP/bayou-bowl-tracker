...
import { EventScoreByTeam, useEventScoresForBB2 } from '../../lib/swr/event-score';
...
function Table({ scores, isSeeded }: { scores: EventScoreByTeam[], isSeeded: boolean }) {
    return (
        <>
            ...
            <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded bg-white" >
                <div className="rounded-t mb-0 px-4 py-3 border-0">
                    <div className="flex flex-wrap items-center">
                        <div className="relative w-full px-4 max-w-full flex-grow flex-1">
                            <h3 className="font-semibold text-lg text-blueGray-600 text-center" >
                                {isSeeded ? "Seeded Leaderboard" : "Open Leaderboard"}
                            </h3>
                        </div>
                    </div>
                    <div className="flex flex-wrap items-center">
                ...
                <div className="block w-full overflow-x-auto">
                    <table className="items-center w-full bg-transparent border-collapse">
                        ...
                        <tbody>
                            {scores?.sort(firstBy('totalScore', 'desc').thenBy('totalRounds').thenBy('kills', 'desc').thenBy('bounties', 'desc')).map((score, index) =>
                                <tr key={score.teamId}>
...
function Page() {
    const { bb2EventScores, isLoading } = useEventScoresForBB2({ suspense: false, refreshInterval: 60000 })

    return (
        <div className="flex flex-wrap w-full mt-4 mr-3 ml-3">
            <div className="w-full md:w-6/12 pr-3">
                        ...
                        <div className="items-center flex flex-wrap">
                            <div className="w-full lg:w-6/12 px-4 ml-auto mr-auto text-center">
                                <div className="pr-12 mt-10">
                                    <h1 className="mt-4 text-6xl uppercase font-bold text-blueGray-200">
                                        Bayou Bowl II Leaderboards
                                    </h1>
                                </div>
                            </div>
                        </div>
